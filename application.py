...
from loginck import login_check
...
APPLICATION_NAME = "Complaint-App"
...
def gconnect():
    ...
    if result['issued_to'] != CLIENT_ID:
        response = make_response(
            json.dumps("Token client error"), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('User in use.'),
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def gdisconnect():
    ...
    if result['status'] == '200':
        ...
        response = redirect(url_for('showComplaints'))
...
@app.route('/complaints/<path:category_name>/items/')
...
# Specific Complaint View
@app.route('/complaints/<path:category_name>/<path:item_name>/')
def showItem(category_name, item_name):
    item = session.query(Items).filter_by(name=item_name).one()
    creator = getUserInfo(item.user_id)
    categories = session.query(Category).order_by(asc(Category.name))
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('open_itemsdetail.html',
                               item=item,
...
# Add Complaint Type
@app.route('/complaints/addcategory', methods=['GET', 'POST'])
@login_check
def addCategory():
...
# Edit Complaint Type
@app.route('/complaints/<path:category_name>/edit', methods=['GET', 'POST'])
@login_check
...
@app.route('/complaints/<path:category_name>/delete', methods=['GET', 'POST'])
@login_check
...
# Add Individual Complaint
@app.route('/complaints/add', methods=['GET', 'POST'])
@login_check
def addItem():
    addTime = datetime.datetime.now()
    addTime = addTime.replace(second=0, microsecond=0)
    categories = session.query(Category).all()
    if request.method == 'POST':
        newItem = Items(
            name=request.form['name'] + " - Date: " + str(addTime),
...
# Edit Individual Complaint
@app.route('/complaints/<path:category_name>/<path:item_name>/edit', methods=['GET', 'POST'])
@login_check
...
# Delete Individual Complaint
@app.route('/complaints/<path:category_name>/<path:item_name>/delete', methods=['GET', 'POST'])
@login_check
...
@app.route('/complaints/categories/JSON')
def categoriesJSON():
    categories = session.query(Category).all()
    return jsonify(categories=[c.serialize for c in categories])

@app.route('/complaints/items/JSON')
def itemsJSON():
    items = session.query(Items).all()
    return jsonify(items=[i.serialize for i in items])

@app.route('/complaints/<path:category_name>/items/JSON')
def categoryItemsJSON(category_name):
    category = session.query(Category).filter_by(name=category_name).one()
    items = session.query(Items).filter_by(category=category).all()
    return jsonify(items=[i.serialize for i in items])

@app.route('/complaints/<path:category_name>/<path:item_name>/JSON')
